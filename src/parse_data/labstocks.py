...
database = MySQLDatabase('labstocks_db', host='127.0.0.1', user='superuser', port=3306) 
...
class Plasmids(BaseModel):
    author_pl = ForeignKeyField(db_column='Author_pl', null=True, rel_model=LabMembers, to_field='id')
    ...
    ekb_no = IntegerField(db_column='EKB_no', null=True)
    ...
    name_plasmid = CharField(db_column='Name_plasmid', null=True)
    other_names = CharField(db_column='Other_names', null=True)
    promoter = CharField(db_column='Promoter', null=True)
    reference_pl = CharField(db_column='Reference_pl', null=True)
    ...
    date_plasmid = DateField(null=True)
